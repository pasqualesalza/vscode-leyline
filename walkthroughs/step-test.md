## Test and start typing

Verify that Leyline can reach your endpoint before writing any code.

[Test Connection](command:leyline.testConnection)

If the test succeeds, you're ready. Open any source file and start typing — ghost text completions appear after ~300 ms.

- **Tab** — accept the completion
- **Escape** — dismiss
- **Alt+\\** — force-trigger on demand
- **Status bar** — click to open the Leyline menu

Check [Output → Leyline](command:workbench.action.output.toggleOutput) for logs if completions don't appear.
