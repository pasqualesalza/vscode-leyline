...
endpoints. Bring your own API key — works with Codestral (Mistral) and Ollama.
...
- **Multi-provider** — Codestral and Ollama out of the box
...
- **Configurable** — context window, debounce, max tokens per provider
...
2. Run **Leyline: Select Provider** to choose between Codestral and Ollama
3. _(Codestral only)_ Run **Leyline: Set API Key** and paste your API key
...
| `Leyline: Show Menu`               | Quick access to all Leyline actions   |
...
| --- | --- | --- | --- |
| `leyline.provider` | `string` | `"codestral"` | Completion provider (`codestral` or `ollama`) |
| `leyline.codestral.endpoint` | `string` | `""` | Codestral API endpoint (default: https://codestral.mistral.ai) |
| `leyline.codestral.model` | `string` | `""` | Codestral model (default: `codestral-latest`) |
| `leyline.codestral.maxTokens` | `number` | `256` | Max tokens for Codestral |
| `leyline.ollama.endpoint` | `string` | `""` | Ollama endpoint (default: http://localhost:11434) |
| `leyline.ollama.model` | `string` | `""` | Ollama model (default: `qwen2.5-coder:7b`) |
| `leyline.ollama.maxTokens` | `number` | `256` | Max tokens for Ollama |
...
5. The response is post-processed to strip repetition and prefix duplicates
6. The cleaned completion appears as ghost text — press `Tab` to accept,
   `Escape` to dismiss
