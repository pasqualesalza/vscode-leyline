import { stripBracketOverflow } from "./brackets.js";
...
/**
 * Strip `width` visual columns of leading whitespace from a single line.
 * Returns the line unchanged if it doesn't have enough leading whitespace.
 */
function stripVisualCols(line: string, width: number, tabSize: number): string {
  if (width === 0) return line;
  let cols = 0;
  let i = 0;
  while (i < line.length && cols < width) {
    if (line[i] === "\t") {
  ...
  return cols >= width ? line.slice(i) : line;
}
...
export function stripLeadingIndent(
  ...
  // Fast path: exact string match on first line
  if (first.startsWith(currentLinePrefix)) {
    const prefixWidth = visualWidth(currentLinePrefix, tabSize);
    return lines
      .map((line, idx) =>
        idx === 0
          ? line.slice(currentLinePrefix.length)
          : stripVisualCols(line, prefixWidth, tabSize),
      )
      .join("\n");
  }
  ...
  if (cols === prefixWidth) {
    return lines
      .map((line, idx) =>
        idx === 0 ? line.slice(i) : stripVisualCols(line, prefixWidth, tabSize),
      )
      .join("\n");
  }
...
export function stripRepetition(completion: string): string {
  ...
  for (
    let blockSize = 2;
    blockSize <= Math.min(Math.floor(lines.length / 2), 20);
    blockSize++
  ) {
...
export function postProcess(
  ...
  // 3. Truncate bracket overflow duplicating suffix closers.
  result = stripBracketOverflow(result, suffix, languageId);

  // 4. Suppress completions that duplicate code already in the prefix.
